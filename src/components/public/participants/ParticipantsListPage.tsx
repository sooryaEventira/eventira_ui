...
const ParticipantsListPage: React.FC<ParticipantsListPageProps> = ({ eventUuid, onNavigate: _onNavigate, tagId, participantId }) => {
  ...
  return (
    <div className="space-y-4 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch min-w-0">
        <div className={selectedId ? 'min-w-0 w-full space-y-3 lg:w-1/2' : 'w-full space-y-3'}>
          {filtered.length === 0 ? (
        ...
        {selectedId && isDesktop && (
          <div className="flex min-w-0 w-full flex-col rounded-xl border border-primary bg-primary/5 lg:sticky lg:top-20 lg:w-1/2 lg:self-stretch lg:min-h-[calc(100dvh-10.5rem)]">
            {renderDetail()}
          </div>
        )}
      </div>
